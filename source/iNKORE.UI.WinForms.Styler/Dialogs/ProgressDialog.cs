	...
	public partial class ProgressDialog : Component
	{
        ...
        public ProgressDialog(IContainer container)
		{
			...
			if (!NativeMethods.IsWindowsVistaOrLater)
				Animation = AnimationResource.GetShellAnimation(iNKORE.UI.WinForms.Styler.Dialogs.ShellAnimation.FlyingPapers);
		}
		...
		public string Text
		{
			...
			set
			{
				_text = value;
				if (_dialog != null)
					Marshal.ThrowExceptionForHR(_dialog.SetLine(1, Text, UseCompactPathsForText, IntPtr.Zero));
			}
		}
		...
		public string Description
		{
			...
			set
			{
				_description = value;
				if (_dialog != null)
					Marshal.ThrowExceptionForHR(_dialog.SetLine(2, Description, UseCompactPathsForDescription, IntPtr.Zero));
			}
		}
		...
		/// </summary>
		/// <value>
		/// One of the values of <see cref="iNKORE.UI.WinForms.Styler.Dialogs.ProgressBarStyle"/>. 
		/// The default value is <see cref="iNKORE.UI.WinForms.Styler.Dialogs.ProgressBarStyle.ProgressBar"/>.
		/// </value>
		/// <remarks>
		/// <note>
		...
		///   progress bar will be hidden completely if this property is <see cref="iNKORE.UI.WinForms.Styler.Dialogs.ProgressBarStyle.MarqueeProgressBar"/>.
		/// </note>
		/// <para>
		///   When this property is set to <see cref="iNKORE.UI.WinForms.Styler.Dialogs.ProgressBarStyle.ProgressBar" />, use the <see cref="ReportProgress(int)"/> method to set
		///   the value of the progress bar. When this property is set to <see cref="iNKORE.UI.WinForms.Styler.Dialogs.ProgressBarStyle.MarqueeProgressBar"/>
		...
		/// </para>
		/// <para>
		///   This method has no effect is <see cref="ProgressBarStyle"/> is <see cref="iNKORE.UI.WinForms.Styler.Dialogs.ProgressBarStyle.MarqueeProgressBar"/>
		///   or <see cref="iNKORE.UI.WinForms.Styler.Dialogs.ProgressBarStyle.None"/>.
		/// </para>
		/// </remarks>
		...
		private void RunProgressDialog(IntPtr owner, object argument)
		{
			...
			Marshal.ThrowExceptionForHR(_dialog.SetTitle(WindowTitle));
			if (Animation != null)
				Marshal.ThrowExceptionForHR(_dialog.SetAnimation(_currentAnimationModuleHandle, (ushort)Animation.ResourceId));

			if (CancellationText.Length > 0)
				Marshal.ThrowExceptionForHR(_dialog.SetCancelMsg(CancellationText, null));
			Marshal.ThrowExceptionForHR(_dialog.SetLine(1, Text, UseCompactPathsForText, IntPtr.Zero));
			Marshal.ThrowExceptionForHR(_dialog.SetLine(2, Description, UseCompactPathsForDescription, IntPtr.Zero));
			...
			Marshal.ThrowExceptionForHR(_dialog.StartProgressDialog(owner, null, flags, IntPtr.Zero));
		...
		{
			Marshal.ThrowExceptionForHR(_dialog.StopProgressDialog());
		...
		private void _backgroundWorker_ProgressChanged(object sender, ProgressChangedEventArgs e)
		{
			...
			{
				Marshal.ThrowExceptionForHR(_dialog.SetProgress((uint)e.ProgressPercentage, 100));

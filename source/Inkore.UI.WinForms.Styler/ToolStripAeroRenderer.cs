...
namespace Inkore.UI.WinForms.Styler
{
	...
	public class ToolStripAeroRenderer : ToolStripSystemRenderer
	{
        ...
        public bool FixRendering { get; set; } = true;
        public bool DrawStatusLabelLine { get; set; } = true;
        ...
        private VisualStyleRenderer renderer = new VisualStyleRenderer(VisualStyleElement.Button.PushButton.Normal);
		...
		{
			if (EnsureRenderer() && Theme != ToolbarTheme.None)
			{
        ...
        protected override void OnRenderStatusStripSizingGrip(ToolStripRenderEventArgs e)
        {
            if (EnsureRenderer())
            {
                ...
                Rectangle gripBounds = ((StatusStrip)e.ToolStrip).SizeGripBounds;
                renderer.DrawBackground(e.Graphics, FixRendering ? new Rectangle(gripBounds.X - 4, gripBounds.Y - 3, gripBounds.Width + 4, gripBounds.Height + 3) : gripBounds);
            }
            else
        ...
        protected override void OnRenderToolStripStatusLabelBackground(ToolStripItemRenderEventArgs e)
        {
            if (EnsureRenderer())
            {
                if (DrawStatusLabelLine)
                {
                    base.OnRenderToolStripStatusLabelBackground(e);
                    renderer.SetParameters(VisualStyleElement.Status.Pane.Normal);
                    renderer.DrawBackground(e.Graphics, e.Item.Bounds);
                }
                else
                {
                    base.OnRenderToolStripStatusLabelBackground(e);
        ...
        protected override void OnRenderSeparator(ToolStripSeparatorRenderEventArgs e)
		{
			if (EnsureRenderer())
			{
                if (e.ToolStrip.IsDropDown)
                {
                    ...
                    var rect = new Rectangle(FixRendering ? e.ToolStrip.DisplayRectangle.Left - 5 : e.ToolStrip.DisplayRectangle.Left, 0, e.ToolStrip.DisplayRectangle.Width, e.Item.Height);
                    renderer.DrawBackground(e.Graphics, rect, rect);
                }
                else
                {
                    renderer.SetParameters(!e.Vertical ? VisualStyleElement.ToolBar.SeparatorVertical.Normal : VisualStyleElement.ToolBar.SeparatorHorizontal.Normal);
                    Rectangle rect = new Rectangle(Point.Empty,e.Item.Size);
                    if (FixRendering)
                    {
                        if (!e.Vertical)
                            rect = new Rectangle(1, 0, rect.Width - 3, rect.Height);
                        else
                            rect = new Rectangle(0, 1, rect.Width, rect.Height - 3);
		...
		protected override void OnRenderItemCheck(ToolStripItemImageRenderEventArgs e)
		{
			if (EnsureRenderer())
			{
                ...
                {
                    if (((ToolStripMenuItem)e.Item).CheckState == CheckState.Indeterminate)
                    {
                        renderer.SetParameters(MenuClass, (int)MenuParts.PopupCheck,
                       e.Item.Enabled
                           ? (int)MenuPopupCheckStates.BulletNormal
                           : (int)MenuPopupCheckStates.BulletDisabled);

...
namespace Inkore.UI.WinForms.Styler.Dialogs
{
	...
	public class ExtendedForm : Form
	{
		...
		{
			if (pevent != null && (DesignMode || Glass.IsDwmCompositionEnabled))
			{
				if (DesignMode)
				{
		...
		private void PaintGlassArea(PaintEventArgs pevent, Brush brush)
		{
			...
			else
			{
				...
				if (_glassMargin.Right != 0)
					pevent.Graphics.FillRectangle(brush, new Rectangle(ClientSize.Width - _glassMargin.Right, 0, ClientSize.Width, ClientSize.Height));
				...
				if (_glassMargin.Bottom != 0)
					pevent.Graphics.FillRectangle(brush, new Rectangle(0, ClientSize.Height - _glassMargin.Bottom, ClientSize.Width, ClientSize.Height));
			}
		}

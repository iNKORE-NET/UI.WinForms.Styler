...
namespace Inkore.UI.WinForms.Styler.Controls
{
    ...
    //[Designer(typeof(Inkore.UI.WinForms.Styler.Design.SearchTextBoxDesigner))]
	...
	public partial class SearchTextBox : Control
	{
		...
		public SearchTextBox() {
			//Load defaults (this happens before initializing the component from designer values)
            ...
            //Init rest of control (loads designer values)
			InitializeComponent();
		}
		...
		{
			get {
				if (_inactiveFont == null && Parent != null)
					return Parent.Font;
				else
		...
		public override Font Font {
			set {
				base.Font = value;
				ActiveFont = value;
                InactiveFont = new Font(Font, FontStyle.Italic);
			}
		}
        ...
        /// </summary>
		[Description("Determines the delay in milliseconds between when the text is edited and the search event is raised."),
            Category("Behavior"),
            DefaultValue(DefaultTimerInterval)]
        ...
        /// </summary>
		[Description("Gets or sets whether the control raises a SearchStarted event when the user hits the Enter key on the text box."),
            Category("Behavior"),
            DefaultValue(false)]
